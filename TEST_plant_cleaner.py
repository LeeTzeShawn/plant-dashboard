...
def GL_Cleaner(plant_name, plant_key=None):
    ...
    GL['Category'] = (
        GL['Category']
        .str.replace(r'^COGS\s*-\s*', '', regex=True)   # remove COGS -
        .str.replace(r'\s*CONSUMED$', '', regex=True)   # remove CONSUMED
        .str.strip()
        .str.upper()
    )
    # ✅ Remove footnote markers like [*1]
    GL['Category'] = GL['Category'].str.replace(r'\[\*?\d+\]', '', regex=True)

    # ✅ Clean up any extra spaces left behind
    GL['Category'] = GL['Category'].str.strip()
    GL = GL.reset_index(drop=True)

    # GL['GLAccount_Category'] = pd.concat([GL['A/C CODE'], GL['Category']], axis=1) #The new line I just added
    
    Cleaned_insideConsumptionList_ordered = []
    for word in CONSUMPTION_LIST:
        if word == 'NICKEL':
            new_word = 'Nickel Catalyst' #Nickel Catalyst is inside the Category column
    ...
    clean_list = [str(x).strip().upper() for x in Cleaned_insideConsumptionList_ordered]

    # print(f"This is the Cleaned_insideConsumptionList_ordered: {Cleaned_insideConsumptionList_ordered}")
    # print("Unique GL Categories:")
    # print(GL['Category'].dropna().unique())

    # print("\nConsumption List:")
    # print(Cleaned_insideConsumptionList_ordered)

    GLItems_Inside_ConsumptionList = GL[
        GL['Category_clean'].isin(clean_list)
    ...
    # print("This is the GLItems_Inside_ConsumptionList: ")
    ...
    GLItems_Inside_ConsumptionList['Category'] = GLItems_Inside_ConsumptionList['Category'].astype(str)
    mask = GLItems_Inside_ConsumptionList['Category'].isna() & GLItems_Inside_ConsumptionList['Description'].str.startswith('COGS - ', na=False)

    GLItems_Inside_ConsumptionList.loc[mask, 'Category'] = (
        GLItems_Inside_ConsumptionList.loc[mask, 'Description']
        .str.replace(r'^COGS\s*-\s*', '', regex=True)
        .str.strip()
    )
    mask_ops = (
    GLItems_Inside_ConsumptionList['Category'].isna() &
    GLItems_Inside_ConsumptionList['Description'].str.strip().eq('OPERATING SUPPLIES')
)
    GLItems_Inside_ConsumptionList.loc[mask_ops, 'Category'] = 'OPERATING SUPPLIES'
    mask_ops_partial = (
    GLItems_Inside_ConsumptionList['Category'].isna() &
    GLItems_Inside_ConsumptionList['Description'].str.contains(r'^OP[- ]?SUPPLIES.*EXPENSES', regex=True, na=False)
)

    GLItems_Inside_ConsumptionList.loc[mask_ops_partial, 'Category'] = 'OP-SUPPLIES EXPENSES - OTHERS'
    
    #Standardize the category column
    GLItems_Inside_ConsumptionList['Category'] = GLItems_Inside_ConsumptionList['Description'].replace({
        'COGS - OPERATING SUPPLIES - P':'OP-SUPPLIES EXPENSES - OTHERS',
        'COGS - CAUSTIC SODA':'OP-SUPPLIES EXPENSES - OTHERS',
        'COGS - HYDROCHLORIC ACID':'OP-SUPPLIES EXPENSES - OTHERS',
        'COGS - POTASIUM HYDROXIDE':'OP-SUPPLIES EXPENSES - OTHERS',
        'COGS - SULFURIC ACID':'OP-SUPPLIES EXPENSES - OTHERS',
        'OPERATING SUPPLIES':'OP-SUPPLIES EXPENSES - OTHERS',
    })

    #Map GL_Account separately
    # GLItems_Inside_ConsumptionList['A/C CODE'] = GLItems_Inside_ConsumptionList['Description'].map({
    #     'COGS - OPERATING SUPPLIES - P':'700500005',
    #     'COGS - CAUSTIC SODA':'700500040',
    #     'COGS - HYDROCHLORIC ACID':'700500120',
    #     'COGS - POTASIUM HYDROXIDE':'700500240',
    #     'COGS - SULFURIC ACID':'700500260',
    #     'OPERATING SUPPLIES':'705360000',
    # })

    
    

    GLItems_Inside_ConsumptionList['Category'] = pd.Categorical(
            GLItems_Inside_ConsumptionList['Category']
        )
...
def getDC_OH_Inside_ConsumptionList(plant_name, plant_key=None): 
    ...
    # print("This the newly created GLItems_Inside_ConsumptionList_DC_OH: ")
    # display(GLItems_Inside_ConsumptionList_DC_OH)
    consumption_df, CONSUMPTION_LIST = get_ConsumptionList(plant_name, plant_key)
    
    # ✅ FIX OP-SUPPLIES mismatch (THIS IS THE KEY FIX)
    # consumption_df['Consumption'] = consumption_df['Consumption'].replace({
    #     'OP-SUPPLIES EXPENSES - OTHERS': 'OP-SUPPLIES EXPENSES - OTHERS'
    # })
    ...
    # GLItems_Inside_ConsumptionList['A/C CODE'] = GL['A/C CODE']
    ...
    #     how='left'
    # )
    # print("This is consumption_df before the merge!!!: ")
    # display(consumption_df)
    # merged_df = merged_df.drop(columns=['Category'])
    
    common_cols = list(set(consumption_df.columns).intersection(GL.columns))
    # print(common_cols)
    print("Plant:", plant_key)
    print("Columns in GLItems_Inside_ConsumptionList:")
    print(GLItems_Inside_ConsumptionList.columns)
    ...
    consumption_df,
    left_on=['Category_clean', 'Product_ID'],
    right_on=['Consumption', 'Product_ID'],
    how='outer'
    )

    # print("THIS is the merged_df right after the merging operation: ")
    # display(merged_df)
    # print('========================================================================')
    # merged_df = consumption_df.merge(
    #     GLItems_Inside_ConsumptionList[
    ...
    # print("This is the merged_df AFTER adding 'Unit_Rate' and 'Allocated_Cost' columns: ")
    # display(merged_df)
    # print("This is merged_df: ")
    # display(merged_df)
    # print('---------------------')
    # print(f"This is the unique 'Category' column values of consumption_df:")
    # print(consumption_df['Category'].value_counts())
    # print()
    # print(f"This is the unique 'Category' column values inside GLItems_Inside_ConsumptionList_DC_OH: ")
    # print(GLItems_Inside_ConsumptionList_DC_OH['Category'].value_counts())

    # print("This is the number of unique Product_IDs")
    # print(GL_agg['Product_ID'].nunique())
    return merged_df
...
# getDC_OH_Inside_ConsumptionList(plantW1, 'plantW1')
...
def get_Total_AllocatedCost_perProduct(df_inside, df_notinside, df_lc, plant_name):
    ...
    #     id_vars=['Product_ID'],
    #     var_name='Cost_Type',
    #     value_name='Allocated_Cost'
    # )

    return result #.sort_values('Allocated_Cost', ascending=False)
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        for sheet_name, plant_df in all_sheets.items():
            if sheet_name in plant_key_mapping:
                ...
                df4 = get_Total_AllocatedCost_perProduct(df1, df2, df3, plant_key)

                #Save immediately
                sheet_name1 = f"{plant_key}_Inside"[:31]
                sheet_name2 = f"{plant_key}_NotInside"[:31]
                sheet_name3 = f"{plant_key}_LC"[:31]
                sheet_name4 = f"{plant_key}_TotalAllocatedCost"[:31]
